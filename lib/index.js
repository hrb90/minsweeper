...
const render = (vBoard, game) => {
  ...
  if (!(game.canPlay || game.won)) {
    $l(".resetter").removeClass("smile");
    $l(".resetter").addClass("dead");
  }
};
...
const addListeners = (vBoard, game) => {
  ...
  $l(".tile").on("dblclick", e => {
    game.superclick([+(e.target.dataset.row), +(e.target.dataset.col)]);
    render(vBoard, game);
  })
  $l(".resetter").on("click", e => {
    game.reset();
    e.target.className = "resetter smile";
    render(vBoard, game);
  });
};
